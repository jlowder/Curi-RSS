...
function normalizeLlmEndpoint(endpoint: string): string {
  ...
  const shouldNormalize =
    ...
    endpoint.endsWith('/v1/') ||
    baseUrlsToNormalize.some(baseUrl => endpoint.includes(baseUrl)) && !endpoint.includes('/chat/completions');
...
async function extractFullArticleContent(url: string): Promise<{content: string | null, imageUrl: string | null}> {
  let html = '';
  let browser;
  ...
  try {
    ...
    for (const selector of contentSelectors) {
      ...
      if (element.length > 0) {
        ...
        element.find('script, style, nav, aside, .advertisement, .social-share, .related-posts, .comments, .sidebar, .navigation, .nav, .footer, .header, .ad, .ads, svg').remove();
        
        // Get content and clean it up
        const rawContent = element.html() || '';
        const textContent = element.text().trim();
        
        console.log(`Selector "${selector}" found content length: ${textContent.length} chars`);
        
        if (textContent.length > 500) { // Only use if substantial text content
    ...
    if (!articleContent || $('<div>').html(articleContent).text().trim().length < 500) {
      ...
      paragraphs.each((_, p) => {
        ...
        if (parentText.length > maxLength && parentText.length > 300) {
          parent.find('script, style, nav, aside, .advertisement, .social-share, .sidebar, .navigation, .nav, .footer, .header, .ad, .ads').remove();
      ...
      if (!articleContent || $('<div>').html(articleContent).text().trim().length < 300) {
        console.log('Trying body content as last resort...');
        const bodyContent = $('body').clone();
        bodyContent.find('script, style, nav, aside, .advertisement, .social-share, .sidebar, .navigation, .nav, .footer, .header, .ad, .ads, .menu').remove();
        const bodyText = bodyContent.text().trim();
        if (bodyText.length > 1000) {
          // Extract just the paragraphs from body
          const bodyParagraphs = bodyContent.find('p').map((_, el) => $(el).prop('outerHTML')).get().join('');
          if (bodyParagraphs && $('<div>').html(bodyParagraphs).text().trim().length > 500) {
            articleContent = bodyParagraphs;
            console.log(`Using body paragraphs with ${$('<div>').html(bodyParagraphs).text().trim().length} chars`);
          }
        }
      }
    }
    ...
    const finalTextLength = articleContent ? $('<div>').html(articleContent).text().trim().length : 0;
    console.log(`Final extraction result: ${finalTextLength} characters of text content`);
...
async function ensureFullArticleContent<T extends Article>(article: T): Promise<T> {
  const isContentInsufficient = !article.content || article.content.length < 1000 || article.content === article.description;

  if (isContentInsufficient) {
    console.log(`Insufficient content for article "${article.title}". Fetching from source, "${article.url}".`);
    try {
      const { content: newContent } = await extractFullArticleContent(article.url);
      if (newContent && newContent.length > (article.content?.length || 0)) {
        console.log(`Successfully extracted new content with length ${newContent.length}.`);
        const updatedArticle = await storage.updateArticleContent(article.id, { content: newContent });
        if (updatedArticle) {
	...
	if (newContent) {
      ...
      console.error(`Failed to fetch full content for article: ${article.title}`, error);
    }
  }
  return article;
}
...
export async function registerRoutes(app: Express): Promise<Server> {
      ...
      try {
        ...
        if (feed.items && feed.items.length > 0) {
          ...
          for (const item of articlesToProcess) {
            ...
            const existingArticle = await storage.getArticleByUrl(item.link || "");
            ...
            if (!existingArticle && item.link) {
              ...
              try {
                console.log(`Extracting full content for new article: ${item.title}`);
                const { content, imageUrl: extractedImageUrl } = await extractFullArticleContent(item.link);
                ...
                console.error('Failed to extract full content for new article:', extractError);
              }
        ...
        res.status(400).json({ error: "Invalid RSS feed URL or feed cannot be parsed" });
      }
    } catch (error) {
        ...
        if (parsedFeed.items && parsedFeed.items.length > 0) {
          for (const item of parsedFeed.items.slice(0, 20)) {
            ...
            if (!existingArticle && item.link) {
              ...
              // Extract full content and enhanced image if needed
              try {
                console.log(`Extracting full content for refreshed article: ${item.title}`);
                const { content, imageUrl: extractedImageUrl } = await extractFullArticleContent(item.link);
                ...
                console.error('Failed to extract full content for refreshed article:', extractError);
              }
  ...
  app.post("/api/articles/delete-unknown-age", async (req, res) => {
    try {
      ...
      console.log(`Manual cleanup completed: deleted ${deletedCount} articles with unknown age.`);
      res.json({
        success: true,
        deletedCount,
  ...
  app.post("/api/articles/cleanup", async (req, res) => {
    try {
      ...
      console.log(`Manual cleanup completed: deleted ${deletedCount} old read articles`);
      res.json({ 
        success: true, 
        deletedCount,
  ...
  app.get("/api/articles/:id", async (req, res) => {
    try {
      ...
      console.log(`Article "${article.title}" has ${article.content?.length || 0} chars of content`);
      
      if (!article.content || article.content.length < 300) {
        console.log(`Attempting to extract full content for article: ${article.title}`);
        console.log(`Article URL: ${article.url}`);
        try {
          const { content, imageUrl } = await extractFullArticleContent(article.url);
          ...
          if (content && content.length > 200) {
            ...
            if (updatedArticle) {
              ...
              console.log(`Returning updated article with content length: ${updatedArticle.content?.length || 0}`);
              return res.json(updatedArticle);
            } else {
              console.log(`Failed to update article in database`);
            }
          } else {
            console.log(`Extraction failed or produced insufficient content: ${content?.length || 0} chars`);
          }
        } catch (extractError) {
          console.error('Failed to extract content for article:', extractError);
        }
      } else {
        console.log(`Article already has sufficient content (${article.content.length} chars)`);
      }
    ...
    try {
      ...
      for (const feed of feeds) {
        try {
          ...
          if (parsedFeed.items && parsedFeed.items.length > 0) {
            for (const item of parsedFeed.items.slice(0, 10)) { // Limit to 10 per feed
              if (isArticleTooOld(item.pubDate)) {
                continue;
              }
              const existingArticle = await storage.getArticleByUrl(item.link || "");
  ...
  app.get("/api/settings/email-config", async (req, res) => {
    try {
      const keys = ["fromAddress", "toAddress", "toAddressAlternate", "toAddressAlternate2", "smtpHost", "smtpPort", "smtpUser", "smtpPass", "smtpSecure"];
      const settings = await Promise.all(keys.map(key => storage.getSetting(key)));
      const config = {
  ...
  app.get("/api/settings/publishing", async (req, res) => {
    try {
      ...
      const settings = await Promise.all(keys.map(key => storage.getSetting(`pub_${key}`)));
      const config = {
  ...
  app.post("/api/articles/:id/summarize", async (req, res) => {
    try {
      ...
      if (!article.content) {
        return res.status(404).json({ error: "Article content not found even after fetching" });
      }
  ...
  app.post("/api/queue/publish", async (req, res) => {
    ...
    try {
      ...
      const keys = ["fromAddress", "smtpHost", "smtpPort", "smtpUser", "smtpPass", "smtpSecure"];
      const settings = await Promise.all(keys.map(key => storage.getSetting(key)));
      const config = {
        fromAddress: settings[0],
        smtpHost: settings[1],
      ...
      if (!toAddress || !config.fromAddress) {
        return res.status(400).json({ error: "Recipient and From address are not configured" });
      }
      ...
      res.status(500).json({ error: "Failed to publish queue", details: errorMessage });
    } finally {
      // Cleanup
      await fs.unlink(htmlPath).catch(e => console.error("Error cleaning up html file:", e));
      await fs.unlink(epubPath).catch(e => console.error("Error cleaning up epub file:", e));
      await fs.unlink(mdPath).catch(e => console.error("Error cleaning up md file:", e));
      await fs.unlink(pdfPath).catch(e => console.error("Error cleaning up pdf file:", e));
    }
  });
