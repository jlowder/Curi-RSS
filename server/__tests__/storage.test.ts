...
describe("Storage - Unread Count Consistency", () => {
  ...
  it("should handle NULL isRead values consistently between count and filter operations", async () => {
    ...
    const unreadArticles = await storage.getFilteredArticles(
      undefined,
      feed.id,
      "unread",
    );
    ...
    expect(unreadArticles.some((a) => a.title === "Unread Article 1")).toBe(
      true,
    );
    expect(unreadArticles.some((a) => a.title === "NULL Read Article")).toBe(
      true,
    );
  ...
  it("should maintain consistency when marking individual articles as read", async () => {
    ...
    const articleWithNull = await db
      .insert(schema.articles)
      .values({
        id: "null-read-test",
        feedId: feed.id,
